...
class QrSegment {
  ...
  static makeNumeric(t) {
    if (!QrSegment.isNumeric(t)) throw new RangeError("String contains non-numeric characters");
    const e = [];
  ...
  static makeAlphanumeric(t) {
    if (!QrSegment.isAlphanumeric(t)) throw new RangeError("String contains unencodable characters in alphanumeric mode");
    const e = [];
    let r;
  ...
  static makeSegments(t) {
    return "" === t ? [] : QrSegment.isNumeric(t) ? [QrSegment.makeNumeric(t)] : QrSegment.isAlphanumeric(t) ? [QrSegment.makeAlphanumeric(t)] : [QrSegment.makeBytes(QrSegment.toUtf8ByteArray(t))];
  }
  static makeEci(t) {
    const e = [];
  ...
  static isNumeric(t) {
    return QrSegment.NUMERIC_REGEX.test(t);
  }
  static isAlphanumeric(t) {
    return QrSegment.ALPHANUMERIC_REGEX.test(t);
  }
  getData() {
...
class QrCode {
  constructor(t, e, r, o) {
    ...
    let s = o;
    if (this.version = t, this.errorCorrectionLevel = e, t < QrCode.MIN_VERSION || t > QrCode.MAX_VERSION) throw new RangeError("Version value out of range");
  ...
  static encodeText(t, e) {
    ...
    return QrCode.encodeSegments(r, e);
  }
  ...
  static encodeSegments(t, e, r = 1, o = 40, s = -1, n = true) {
    if (!(QrCode.MIN_VERSION <= r && r <= o && o <= QrCode.MAX_VERSION) || s < -1 || s > 7) throw new RangeError("Invalid value");
    let i, a;
    for (i = r; ; i++) {
      const r2 = 8 * QrCode.getNumDataCodewords(i, e), s2 = QrSegment.getTotalBits(t, i);
      if (s2 <= r2) {
        a = s2;
        break;
      }
    ...
    let h = e;
    for (const t2 of [Ecc.MEDIUM, Ecc.QUARTILE, Ecc.HIGH]) n && a <= 8 * QrCode.getNumDataCodewords(i, t2) && (h = t2);
    const l = [];
    for (const e2 of t) {
    ...
    assert(l.length === a);
    const d = 8 * QrCode.getNumDataCodewords(i, h);
  ...
  addEccAndInterleave(t) {
    ...
    if (t.length !== QrCode.getNumDataCodewords(e, r)) throw new RangeError("Invalid argument");
    const o = QrCode.NUM_ERROR_CORRECTION_BLOCKS[r.ordinal][e], s = QrCode.ECC_CODEWORDS_PER_BLOCK[r.ordinal][e], n = Math.floor(QrCode.getNumRawDataModules(e) / 8), i = o - n % o, a = Math.floor(n / o), h = [], l = QrCode.reedSolomonComputeDivisor(s);
    for (let e2 = 0, r2 = 0; e2 < o; e2++) {
      const o2 = t.slice(r2, r2 + a - s + (e2 < i ? 0 : 1));
      r2 += o2.length;
      const n2 = QrCode.reedSolomonComputeRemainder(o2, l);
  ...
  drawCodewords(t) {
    if (t.length !== Math.floor(QrCode.getNumRawDataModules(this.version) / 8)) throw new RangeError("Invalid argument");
    let e = 0;
  ...
  getPenaltyScore() {
    ...
    for (let e2 = 0; e2 < this.size; e2++) {
      ...
      for (let n = 0; n < this.size; n++) this.modules[e2][n] === r2 ? (o2++, 5 === o2 ? t += QrCode.PENALTY_N1 : o2 > 5 && t++) : (this.finderPenaltyAddHistory(o2, s), r2 || (t += this.finderPenaltyCountPatterns(s) * QrCode.PENALTY_N3), r2 = this.modules[e2][n], o2 = 1);
      t += this.finderPenaltyTerminateAndCount(r2, o2, s) * QrCode.PENALTY_N3;
    }
    for (let e2 = 0; e2 < this.size; e2++) {
      let r2 = false, o2 = 0;
      const s = [0, 0, 0, 0, 0, 0, 0];
      for (let n = 0; n < this.size; n++) this.modules[n][e2] === r2 ? (o2++, 5 === o2 ? t += QrCode.PENALTY_N1 : o2 > 5 && t++) : (this.finderPenaltyAddHistory(o2, s), r2 || (t += this.finderPenaltyCountPatterns(s) * QrCode.PENALTY_N3), r2 = this.modules[n][e2], o2 = 1);
      t += this.finderPenaltyTerminateAndCount(r2, o2, s) * QrCode.PENALTY_N3;
    }
    for (let e2 = 0; e2 < this.size - 1; e2++) for (let r2 = 0; r2 < this.size - 1; r2++) {
      const o2 = this.modules[e2][r2];
      o2 === this.modules[e2][r2 + 1] && o2 === this.modules[e2 + 1][r2] && o2 === this.modules[e2 + 1][r2 + 1] && (t += QrCode.PENALTY_N2);
    }
    let e = 0;
    for (const t2 of this.modules) e = t2.reduce((t3, e2) => t3 + (e2 ? 1 : 0), e);
    const r = this.size * this.size, o = Math.ceil(Math.abs(20 * e - 10 * r) / r) - 1;
    return assert(o >= 0 && o <= 9), t += o * QrCode.PENALTY_N4, assert(t >= 0 && t <= 2568888), t;
  }
  ...
  static getNumRawDataModules(t) {
    if (t < QrCode.MIN_VERSION || t > QrCode.MAX_VERSION) throw new RangeError("Version number out of range");
    let e = (16 * t + 128) * t + 64;
    if (t >= 2) {
  ...
  static getNumDataCodewords(t, e) {
    return Math.floor(QrCode.getNumRawDataModules(t) / 8) - QrCode.ECC_CODEWORDS_PER_BLOCK[e.ordinal][t] * QrCode.NUM_ERROR_CORRECTION_BLOCKS[e.ordinal][t];
  }
  static reedSolomonComputeDivisor(t) {
    ...
    let r = 1;
    for (let o = 0; o < t; o++) {
      for (let t2 = 0; t2 < e.length; t2++) e[t2] = QrCode.reedSolomonMultiply(e[t2], r), t2 + 1 < e.length && (e[t2] ^= e[t2 + 1]);
  ...
  static reedSolomonComputeRemainder(t, e) {
    ...
    for (const o of t) {
      ...
      r.push(0), e.forEach((e2, o2) => {
        r[o2] ^= QrCode.reedSolomonMultiply(e2, t2);
      });
    }
    return r;
  }
